...
index=$((index + 1))
print_check_point $index "$check_point"
passminlen=$(grep PASS_MIN_LEN /etc/login.defs | grep -v '^#')
...
index=$((index + 1))
print_check_point $index "$check_point"
passwarn=$(grep PASS_WARN_AGE /etc/login.defs | grep -v '^#')
...
flag=0
info=$(grep password /etc/pam.d/system-auth | grep requisite)
print_info "$info"
...
if [ -n "$info" ]; then
	...
	echo "$dcredit"
	if [ "${dcredit:-0}" -eq 1 ] && [ "${ucredit:-0}" -eq 1 ] && [ "${lcredit:-0}" -eq 1 ] && [ "${ocredit:-0}" -eq 1 ]; then
...
line_minlen=$(grep minlen /etc/security/pwquality.conf | grep -v '^#')
line_minclass=$(grep minclass /etc/security/pwquality.conf | grep -v '^#')
...
umask1=$(grep umask /etc/csh.cshrc | /bin/awk -F 'umask' 'NR==1{print $2}')
...
umask3=$(grep umask /etc/bashrc | /bin/awk -F 'umask' 'NR==2{print $2}')
...
umask4=$(grep umask /etc/profile | /bin/awk -F 'umask' 'NR==2{print $2}')
...
print_info "检查 $services_file 文件权限："
if [ -f "$shadow_file" ]; then
	services_stat=$(stat -c %a $services_file)
	print_info "$services_file 的权限应该大于等于644，实际为：===>""$services_stat"
	if [ "$services_stat" -ge 644 ]; then
		echo "3.2.4	检查 $services_file 文件权限	一般	自行判断	参考父项3.2	>=644	$services_stat	TRUE		" >>"$csvFile"
		print_pass
	else
		echo "3.2.4	检查 $services_file 文件权限	一般	自行判断	参考父项3.2	>=644	$services_stat	FAIL		" >>"$csvFile"
		flag1=0
		print_fail
	fi
else
	echo "3.2.4	检查 $services_file 文件权限	一般	自行判断	参考父项3.2	>=644	文件不存在	TRUE		" >>"$csvFile"
	print_white "文件不存在！"

fi

security_file="/etc/security"
print_info "检查 $security_file 目录权限："
if [ -d "$security_file" ]; then
	security_stat=$(stat -c %a $security_file)
	print_info " $security_file 的权限应该大于等于600，实际为：===>""$security_stat"
	if [ "$security_stat" -ge 600 ]; then
		echo "3.2.5	检查 $security_file 目录权限	一般	自行判断	参考父项3.2	>=600	$security_stat	TRUE		" >>"$csvFile"
		print_pass
	else
		echo "3.2.5	检查 $security_file 目录权限	一般	自行判断	参考父项3.2	>=600	$security_stat	FAIL		" >>"$csvFile"
		flag1=0
		print_fail
	fi
else
	echo "3.2.5	检查 $security_file 目录权限	一般	自行判断	参考父项3.2	>=600	目录不存在	TRUE		" >>"$csvFile"
	print_white "目录不存在！"

fi

passwd_file="/etc/passwd"
print_info "检查 $passwd_file 文件权限："
if [ -f "$passwd_file" ]; then
	passwd_stat=$(stat -c %a $passwd_file)
	print_info " $passwd_file 的权限应该大于等于644，实际为：===>""$passwd_stat"
	if [ "$passwd_stat" -ge 644 ]; then
		echo "3.2.6	检查 $passwd_file 文件权限	一般	自行判断	参考父项3.2	>=644	$passwd_stat	TRUE		" >>"$csvFile"
		print_pass
	else
		echo "3.2.6	检查 $passwd_file 文件权限	一般	自行判断	参考父项3.2	>=644	$passwd_stat	FAIL		" >>"$csvFile"
		flag1=0
		print_fail
	fi
else
	echo "3.2.6	检查 $passwd_file 文件权限	一般	自行判断	参考父项3.2	>=644	文件不存在	TRUE		" >>"$csvFile"
	print_white "文件不存在！"

fi

file_rc6="/etc/rc6.d"
print_info "检查 $file_rc6 目录权限："
if [ -d "$file_rc6" ]; then
	rc6_stat=$(stat -c %a $file_rc6)
	print_info " $file_rc6 的权限应该大于等于750，实际为：===>""$rc6_stat"
	if [ "$rc6_stat" -ge 750 ]; then
		echo "3.2.7	检查 $file_rc6 目录权限	一般	自行判断	参考父项3.2	>=750	$rc6_stat	TRUE		" >>"$csvFile"
		print_pass
	else
		echo "3.2.7	检查 $file_rc6 目录权限	一般	自行判断	参考父项3.2	>=750	$rc6_stat	FAIL		" >>"$csvFile"
		flag1=0
		print_fail
	fi
else
	echo "3.2.7	检查 $file_rc6 目录权限	一般	自行判断	参考父项3.2	>=750	目录不存在	TRUE		" >>"$csvFile"
	print_white "目录不存在！"

fi

file_rc0="/etc/rc0.d"
print_info "检查 $file_rc0 目录权限："
if [ -d "$file_rc0" ]; then
	rc0_stat=$(stat -c %a $file_rc0)
	print_info " $file_rc0 的权限应该大于等于750，实际为：===>""$rc0_stat"
	if [ "$rc0_stat" -ge 750 ]; then
		echo "3.2.8	检查 $file_rc0 目录权限	一般	自行判断	参考父项3.2	>=750	$rc0_stat	TRUE		" >>"$csvFile"
		print_pass
	else
		echo "3.2.8	检查 $file_rc0 目录权限	一般	自行判断	参考父项3.2	>=750	$rc0_stat	FAIL		" >>"$csvFile"
		flag1=0
		print_fail
	fi
else
	echo "3.2.8	检查 $file_rc0 目录权限	一般	自行判断	参考父项3.2	>=750	目录不存在	TRUE		" >>"$csvFile"
	print_white "目录不存在！"

fi

file_rc1="/etc/rc1.d"
print_info "检查 $file_rc1 目录权限："
if [ -d "$file_rc1" ]; then
	rc1_stat=$(stat -c %a $file_rc1)
	print_info " $file_rc1 的权限应该大于等于750，实际为：===>""$rc1_stat"
	if [ "$rc1_stat" -ge 750 ]; then
		echo "3.2.9	检查 $file_rc1 目录权限	一般	自行判断	参考父项3.2	>=750	$rc1_stat	TRUE		" >>"$csvFile"
		print_pass
	else
		echo "3.2.9	检查 $file_rc1 目录权限	一般	自行判断	参考父项3.2	>=750	$rc1_stat	FAIL		" >>"$csvFile"
		flag1=0
		print_fail
	fi
else
	echo "3.2.9	检查 $file_rc1 目录权限	一般	自行判断	参考父项3.2	>=750	目录不存在	TRUE		" >>"$csvFile"
	print_white "目录不存在！"

fi

file_rc2="/etc/rc2.d"
print_info "检查 $file_rc2 目录权限："
if [ -d "$file_rc2" ]; then
	rc2_stat=$(stat -c %a $file_rc2)
	print_info " $file_rc2 的权限应该大于等于750，实际为：===>""$rc2_stat"
	if [ "$rc2_stat" -ge 750 ]; then
		echo "3.2.10	检查 $file_rc2 目录权限	一般	自行判断	参考父项3.2	>=750	$rc2_stat	TRUE		" >>"$csvFile"
		print_pass
	else
		echo "3.2.10	检查 $file_rc2 目录权限	一般	自行判断	参考父项3.2	>=750	$rc2_stat	FAIL		" >>"$csvFile"
		flag1=0
		print_fail
	fi
else
	echo "3.2.10	检查 $file_rc2 目录权限	一般	自行判断	参考父项3.2	>=750	目录不存在	TRUE		" >>"$csvFile"
	print_white "目录不存在！"
...
print_info "'应设置重要文件为i属性（如：chattr +i  $passwd_file ），设定文件不能删除、改名、设定链接关系等'"

flag2=1
print_info "检查 $passwd_file 的文件属性"
lsattr_pass=$(lsattr $passwd_file | awk '{ print $1 }' | awk -F- '{print $5}')
lsattr1_pass=$(lsattr $passwd_file)
print_info " $passwd_file 的属性值为:""$lsattr1_pass"
if [ "$lsattr_pass"x = "i"x ]; then
	echo "3.3.1	检查 $passwd_file 的文件属性	一般	建议调整	参考父项3.3	i	$lsattr_pass	TRUE	 	" >>"$csvFile"
	print_pass
else
	echo "3.3.1	检查 $passwd_file 的文件属性	一般	建议调整	参考父项3.3	i	$lsattr_pass	FAIL	 	" >>"$csvFile"
	flag2=0
	print_fail
fi
...
index=$((index + 1))
print_check_point $index "$check_point"
tmp=$(grep umask /etc/login.defs | grep -v '^#')
tmp1=$(grep UMASK /etc/login.defs | grep -v '^#')
...
index=$((index + 1))
print_check_point $index "$check_point"
banner1=$(grep Banner /etc/ssh/sshd_config)
...
if [ -z "$banner1" ]; then
	...
	if [ -n "$banner2" ]; then
		...
		manual=$((manual + 1))
		path=$(grep Banner /etc/ssh/sshd_config | awk '{print $2}')
...
tmp=$(grep '/var/log/messages' /etc/rsyslog.conf | grep -E '\*.info;mail.none;authpriv.none;cron.none' | grep -v '^#')
...
tmp=$(grep /var/log/secure '/etc/rsyslog.conf' | grep -E 'authpriv'.\('info|\*'\) | grep -v '^#')
...
index=$((index + 1))
print_check_point $index "$check_point"
Protocol=$(grep -i Protocol /etc/ssh/sshd_config | grep -Ev ^\# | awk '{print $2}')
#PermitRootLogin=`grep -i PermitRootLogin /etc/ssh/sshd_config |  grep -Ev ^\# | awk '{print $2}'`
...
在$services_file文件中，注释掉 telnet 23/tcp 一行(如不生效重启telnetd服务或xinetd服务或系统，例如，Red Hat 上重启xinetd：service xinetd restart，根据实际情况操作)"
print_info "$msg"
manual=$((manual + 1))
...
if [ -z "$tmp" ]; then
	...
	root=$(grep root /etc/vsftpd/ftpusers | grep -v '^#')
	if [ -n "$root" ]; then
		echo "5.4	检查是否禁止root用户登录FTP	一般	建议调整	由于root用户权限过大，容易导致系统文件误删除。此检查项建议调整	null	略	TRUE		" >>"$csvFile"
		pass=$((pass + 1))
		_echo_pass
...
TMOUT=$(grep -i TMOUT /etc/profile | grep -v '^#')
if [ -z "$TMOUT" ]; then
...
soft=$(grep soft /etc/security/limits.conf | grep core | grep 0 | grep '^\*')
hard=$(grep soft /etc/security/limits.conf | grep hard | grep core | grep 0 | grep '^\*')
...
print_info "实际检测值为:"
HISTSIZE=$(grep ^HISTSIZE /etc/profile)
HISTFILESIZE=$(grep ^HISTFILESIZE /etc/profile)
if [ -n "$HISTSIZE" ] && [ -n "$HISTFILESIZE" ]; then
	HISTSIZE=$(grep ^HISTSIZE /etc/profile | awk -F "=" '{print $2}')
	HISTFILESIZE=$(grep ^HISTFILESIZE /etc/profile | awk -F "=" '{print $2}')
...
pam_rootok=$(grep auth /etc/pam.d/su | grep sufficient | grep pam_rootok.so | grep -v '^#')
pam_wheel=$(grep auth /etc/pam.d/su | grep pam_wheel.so | grep group=wheel | grep -v '^#')
print_info "实际配置如下:"
...
msg="请手动检查文件文件 $passwd_file ，/etc/shadow，并使用命令：usermod -s /sbin/nologin username"
print_info "$msg"
manual=$((manual + 1))
...
index=$((index + 1))
print_check_point $index "$check_point"
line=$(grep password /etc/pam.d/system-auth | grep sufficient | grep pam_unix.so | grep remember | grep -v '^#')
...
multi=$(grep multi /etc/host.conf)
if [ -n "$multi" ]; then
...
index=$((index + 1))
print_check_point $index "$check_point"
tmp=$(grep "Alias=ctrl-alt-del.target" /usr/lib/systemd/system/ctrl-alt-del.target | grep -v '^#')
...
rhost=$(locate .rhost | grep -E 'rhost$')
equiv=$(locate .netrc | grep -E 'netrc$')
equiv=$(locate .equiv | grep -E 'hosts.equiv$')
...
ntpd=$(pgrep "ntpd" | grep -v grep)
print_info "$ntpd"
if [ -n "$ntpd" ]; then
	server=$(grep ^server /etc/ntp.conf)
	print_info "==> servers <=="
	print_info "$server"
	if [ -n "$server" ]; then
...
if [ -z "$tmp" ]; then
	...
	chroot_list_file=$(grep ^chroot_list_file=/etc/vsftpd/chroot_list /etc/vsftpd/vsftpd.conf)
